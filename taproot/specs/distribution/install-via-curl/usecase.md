# Behaviour: User installs rpnpad via curl installer
...
2. User runs `curl --proto '=https' --tlsv1.2 -LsSf https://github.com/OWNER/rpnpad/releases/latest/download/rpnpad-installer.sh | sh`.
...
9. User runs `rpnpad` and the calculator starts.
...
- **Steps:**
  ...
  2. User runs `rpnpad` immediately without any PATH adjustment.
...
## Postconditions
- The rpnpad binary is present at `~/.cargo/bin/rpnpad` (or custom location)
- Once `~/.cargo/bin/` is on PATH, `rpnpad` launches the calculator
- No system-level permissions were required (user-space install)
- No Rust toolchain was installed

## Error Conditions
- **Unsupported platform detected**: The installer script prints an error naming the detected OS/arch and exits without installing; user must build from source with `cargo install rpnpad`.
...
- Then the rpnpad binary is placed at `~/.cargo/bin/rpnpad` without requiring sudo or a Rust toolchain
...
- When the installer script runs
- Then it exits with an error naming the detected platform and suggests `cargo install rpnpad` as an alternative
...
- For users who already have Rust installed, `cargo install rpnpad` is a valid alternative (once crates.io publishing is enabled).
